...
public class GameMgr : MonoBehaviourPunCallbacks, IPunObservable
{
    ...
    [HideInInspector] float m_GoWaitGame = 4.0f; // °ÔÀÓ ½ÃÀÛÈÄ Ä«¿îÆ® Text UI

    // °ÔÀÓ Å¸ÀÌ¸Ó
    public Text m_InGameTmText; // 180ÃÊ °ÔÀÓ Å¸ÀÌ¸Ó
    [HideInInspector] public float m_InGameTimer = 180.0f; // 0ÃÊ°¡ µÇ¸é °ÔÀÓ Á¾·á
            ...
            case GameState.GS_Playing:
                {
                    ...
                    if (m_InGameTimer <= 120.0f)
                    {
                        ...
                        if (m_InGameTimer <= 0.0f)
                        {
                            ...
                            {
                                SendGState(GameState.Gs_GameEnd); //<--- ¿©±â¼­´Â Áö±Ý ·ëÀ» ÀÇ¹ÌÇÔ    
                            }
                        }
                    }
                }
    ...
    bool GameStartObserver()
    {
        ...
        if (0.0f < m_GoWaitGame)
        {
            ...
            if (m_GoWaitGame <= 0.0f) // ÀÌ°Ç ÇÑ¹ø¸¸ ¹ß»ýÇÒ °ÍÀÌ´Ù.
            {
                m_RoundCnt++;
                m_WaitTmText.gameObject.SetActive(false);
                m_GoWaitGame = 0.0f;
